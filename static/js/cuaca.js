function getData(location) {
    return fetch("https://data.bmkg.go.id/DataMKG/MEWS/DigitalForecast/DigitalForecast-JawaTengah.xml") // Fetch API Data BMKG
        ...
        .then(data =>{
            ...
            for (var i = 0; i < 4; i++) {
                ...
                dataHours +=`
                        <div class="box">
                            <h3>${hour}.00</h3>
                            <img src="/static/img/cuaca/icon/w_${parseInt(weatherArray[i])}.png" width="75px"><br>
                            <i class="fas fa-temperature-high">Suhu</i> ${temArray[i]}<sup>o</sup>C<br>
                            <i class="fas fa-tint">Kelembapan</i> ${humArray[i]}%<br>
                            <i class="fas fa-location-arrow">Arah Angin</i> ${winddirArray[i]}<sup>o</sup><br>
                            <i class="fas fa-wind">Kecepatan Angin</i> ${windspeedArray[i]} m/s<br>
                        </div>
                `;

                
                if(dates[i-1] != dates[i]){
                    days +=`<div class=""><h3>${timeArray[i][6]}${timeArray[i][7]} ${months[month]} ${year}</h3></div>`
                }
...
function weatherPanel(locations) {
    ...
    for(i = 0; i < locations.length; i++){
        body += `
            <div class="containercuaca"><h1>${locations[i]}</h1></div>
            <div class="containercuaca" id="days${locations[i]}"></div>
            <div class="boxcuaca" id="dataHours${locations[i]}"></div>
        `;
    }
